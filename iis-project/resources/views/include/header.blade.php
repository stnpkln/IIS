<ul class="nav nav-underline justify-content-between px-5 py-3 shadow-sm">
	<div class="d-flex gap-3">
		...
		<li class="nav-item">
			<a class="nav-link {{ Request::is('user*') ? 'active' : '' }}" aria-current="page" href="{{ route('users') }}">Uživatelé</a>
		</li>
	</div>
	<div class="d-flex gap-3">
	@if (session('user'))
		<li class="nav-item">
			<a class="nav-link {{ Request::is('profile') ? 'active' : '' }}" aria-current="page" href="{{ route('profile') }}">Profil</a>
		</li>
		<li class="nav-item">
